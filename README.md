...
> `bbcmicro` romset; the core reads one image built from it.
...
| *ROM* | Pocket SDRAM (`target/pocket/bbcmicro_mem.sv`) | `sim/run_mem.sh`; image byte-identical to MAME's regions |
...
```sh
python3 mra_build.py bbcmicro.mra bbcmicro.zip
```
...
known md5. Copy the result to `Assets/bbcmicro/common/bbcmicro.rom` on the SD card.
