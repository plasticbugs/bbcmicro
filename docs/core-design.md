...
Layout of `bbcmicro.rom` byte by byte, the SDRAM word address of each region,
and the same constants in `target/pocket/bbcmicro_mem.sv` and `sim/tb_mem.cpp`.
...
Every multicycle in `projects/bbcmicro_pocket.sdc`, why everything its filter
